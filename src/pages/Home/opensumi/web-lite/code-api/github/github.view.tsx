  ...
  const renderRateLimit = (key: keyof typeof resources, title: string) => {
    ...
    return (
      <div className="resource">
        <div className="resourceTitle">{title}</div>
        <ul className="rateList">
          <li>
            {localize("github.rate-limit-limit")}
            <span className="rateData">{data.limit}</span>
          </li>
          <li>
            {localize("github.rate-limit-remaining")}
            <span className="rateData">{data.remaining}</span>
          </li>
          <li>
            {localize("github.rate-limit-reset")}
            <span className="rateData">{formateTime(data.reset * 1000)}</span>
          </li>
        </ul>
      </div>
    );
  ...
  return (
    <div className={styles.container}>
      <div className={styles.title}>
        ...
        >
          <i
            className={`${getOctIcon("sync")} ${
              syncing ? "octicon-animation-spin" : ""
            }`}
          ></i>
        </span>
      </div>
